  ...
  for (const row of csv.data) {
    if (row.operation.toLowerCase() === "buy") {
      transactions.push({
        ...
        transactionTicks: (new Date(row.time).getTime() / 1000).toString(),
        transactionDate: new Date(row.time).toUTCString(),
        ...
        feeAmount: parseFloat(row.fee || "0"),
      ...
      if (ratePair) {
        ...
        transactions.push({
          ...
          feeAmount: Math.abs(parseFloat(row.fee || "0")),
          feeCurrency: _sent,
          label: row.operation.toLowerCase(),
          description: row.comment,
          wallet: "coincheck",
...
export const convertReserveHistory = async (
  ...
  return csv.data.map((r) => ({
    id:
      r.Id ||
      `${(new Date(r["Time"]).getTime() / 1000).toString()}-${
        r["Trading Currency"]
      }-coincheck_reserve_action`,
    currencyPair: r["Original Currency"] + "#" + r["Trading Currency"],
    transactionTicks: (new Date(r["Time"]).getTime() / 1000).toString(),
    transactionDate: new Date(r["Time"]).toUTCString(),
